...
| `short_of_end` | 読み手側の位置が本の終わりに達していない。**最終ページではない**。終了コード 1 |
| `unsettled` | 画面が止まらない（読み込み中のまま。#109）。**最終ページではない**。終了コード 1 |
| `reload_jumped` | 開き直したら別の位置へ飛んだ。間のページが抜けるので止めた。終了コード 1 |
...
**読み手側の位置が本の終わりに達しているかを見ます**（#79）。画像が変わらなくなっても、
位置が終わりに達していなければ最終ページとは呼びません（`short_of_end`、終了コード 1）。

判定は**割合と絶対量の両方**で、どちらの上限も超えたときだけ「途中」とします。
実測 8 冊で決めました。最後まで撮れた本の「総量 - 位置」は絶対量で小さく、本の長さに
比例しません。
...
**割合だけでは駄目です。** 不足は最終画面 1 枚ぶんのラグに見えるので、割合だけにすると
短い本ほど厳しくなります（不足 9 は総量 450 を切った時点で 2% を超えます）。この標本は
蔵書の**最長側**から採ったもので、実際の蔵書は中央値 106 ページ・最小 11 ページです。
効くかどうかを決めるのは短い本のほうで、そこは 1 冊も測れていません。だから絶対量の
下駄（100 = 実測の最大ラグ 9 の 10 倍）を併用します。

割合は 10% と、**わざと緩めにしてあります**。偽陽性（正常に撮れた本を毎回失敗させる）が
最悪のケースだからです。失敗した本は中間ファイルが残るので、毎回 GB 単位を置いていきます。
次のバッチで `capture_stopped` の `position` / `book_total` を必ず集計してください。

判定に使う位置は**落ち着くまで読み直します**。この地点は再送で最大 `max_retries` 回
キーを押した直後で、一過性のラベルを掴みやすいためです（#53）。

**フッターの単位と本全体の %**（#110）。フッターは `位置92/3433 ● 2%` か `291/292ページ ● 100%` の
どちらかです。ページ番号を持たない本はずっと「位置」。持つ本は、**ページ番号の無い表紙・前付けでは
「位置」、本文に入ると「ページ」**になり、番号も振り直されます（1 押下ごとに記録した実測: 作文技術は
`位置1/3257` → `位置2/3257` → `1/265ページ`、半導体の地政学は `位置1/3494` → `1/292ページ`）。
先頭から撮る本番では**必ず**この切り替わりを通ります。合本では本文の途中でも起きます
（ハリー・ポッター全 7 巻: `位置1/113618` → 5 回送って `1/1358ページ`、455 → 5）。単位の違う 2 つの値を比べると
「飛んだ」「戻った」と読み違えるので、読み値には単位を持たせ、**開き直しの前後と追いつき待ちの
比較は単位が揃っているときだけ**行い、変わっていたら `position_unit_changed` を記録して新しい
単位で基準を取り直します。また合本にページ番号があると `total` は**いま開いている巻**のページ数で、
% だけが本全体を指します（`169/1358ページ ● 3%`）。**% が `position/total` の割合より 10 ポイント以上
低いときだけ**（= total が本全体ではないと分かるときだけ）% を信じ、90 未満なら `short_of_end` です。
% だけで判定しないのは上の段落と同じ理由で、短い本では最終画面 1 枚ぶんのラグが割合で 10% を超える
（9/11 → 82%）ためです。`capture_stopped` には `unit` と `percent` も残します。

位置が読めない本は判断材料が無いので、従来どおり扱います。
...
**1 ページ目が既に表紙なら足しません。** リーダーのページ送りに表紙が入っている本が
あり（マンガはたいていそう）、しかもフル解像度です。そこへストア画像を足すと、同じ絵が
小さく劣化して 2 枚並びます。取得した表紙と 1 ページ目を見比べて、同じ絵なら足しません
（実測: 同じ表紙の差は 1.3〜2.8、表紙と同じデザインの扉は 16.1、表紙と本文は 50 以上）。
**迷ったら足す側に倒します** — 冗長な 1 ページより、表紙が無いほうが困るからです。

`data-old-hires` が無い本（マンガの商品ページに多い）では、**主画像 (`landingImage`)
の中だけ**で一番大きいものを採ります。ページ全体から面積最大を採ると、おすすめ商品の
表紙が選ばれることがあります（実測でこのブロックは 7 個あり、6 個が他商品）。
...
rem まず 1 冊試す。**抜き取りは蔵書に混ぜない**ので作業フォルダへ出す
python scripts\capture_app_books.py ... --library C:\work\probe --limit 1 ^
    --max-pages 8 --allow-partial
```

`--books` の一覧は、バッチのログに出る `book_unsupported` イベント（`exit_code: 8`）の本を
集めて作ります（`title` / `asin` / `format`）。`--dry-run` で対象だけ確かめられます。

進捗は `--state` の CSV に 1 冊ずつ残ります（`status` / `pages` / `stopped_reason` / `detail`）。
**`完了` の本だけを済みとして飛ばす**ので、失敗した本は次に流したときもう一度撮ります。
撮れなかった本が 1 冊でもあれば終了コードは 1 です。
...
| 読書 UI | 上のバー・スライダー・← → は本文（上の余白）をクリックすると出たり消えたりし、<br>**ページにかぶさる**（ページは動かない）。出たまま撮ると上下が隠れるので、**消してから撮る**。<br>固定で削る余白は無し。残るのは右上のウィンドウ操作ボタンだけで、ページ間で変わらない帯として自動で見つける |
| タイトルバー | アプリは「← Kindle」のタイトルバー（48px）を出す表示と、ページを上端まで広げる表示を<br>行き来する（同じ日に両方あった。切り替わる条件は不明）。本ごとに高さを測り、あるときだけ削る。<br>タイトルバーにも ← があるので、ずらして見ないと読書 UI と取り違える |
| 前面化のクリック | `kindle` プロファイルは前面化でウィンドウ左上をクリックするが、ページを上端まで広げる表示では<br>そこが本文で、表示が変わって拡大された断片が撮れた（2448x1360）。クリックしない設定で撮り、<br>撮影中にウィンドウの位置・大きさが変わった本は完成にしない |
| アプリの起動し直し | 一度おかしくなったアプリ（F11 全画面を経たあと）は検索窓に文字が入らなくなり、クリックでも Tab でも<br>戻らない。実行の最初と、本を開けなかったあとに `taskkill` → `shell:AppsFolder` で起動し直す |
| 検索に当たらない | 一覧の題名そのままでは「一致する商品は見つかりませんでした」になる本がある（全角の数字・記号を含む長い題名）。<br>当たらなければ、最初の空白か括弧までの先頭部分 → それを半角に寄せたもの、の順に探し直す。2 冊以上に当たったら開かない |
| Windows 版で非対応 | 表紙をクリックすると「サポートされていないコンテンツ」のダイアログが出る本がある（Cloud Reader にも<br>アプリにも無い本）。ダイアログの暗幕を見て `Windows 版で非対応` にし（アプリは起動し直さない）、読書 UI の異常と取り違えない |
| 試し撮り | `--max-pages` の出力は蔵書にも進捗 CSV にも「完了」として入れない（部分本が完成扱いで固定される）。<br>試すときは `--library` と `--state` の両方を作業フォルダにする |
| ライブラリの位置 | 上に 55px の帯が出たり消えたりする（実測: 「全て」の行の y が日によって 243 と 188）。<br>固定座標だと検索窓のつもりで「Kindle ストア」を押すので、青く反転した「全て」の行を探して、そこからの相対で決める |

**別の本を撮らないための決め事**:

- 検索が 1 冊に絞れなかった本は撮らずに飛ばす
- 開いて先頭まで戻したあと、**表紙を OCR して題名と突き合わせる**。確かめられない本は撮らない
- 操作の前に毎回「ライブラリにいるか」を画面の色（「全て」の青い行）で確かめる（本を開いた
  ままだと Ctrl+A が読書補助機能になるなど、別の操作になってしまうため）
- ウィンドウのプロセス名が `Kindle.exe` であることを確かめる（アプリが落ちていると、
  題名に kindle を含む別のウィンドウを掴んでキーを送ってしまう）
- クリップボードに題名を入れられたことを読み返して確かめる（失敗すると 1 冊前の題名が残り、
  前の本を今の本の題名で保存してしまう）
- 撮れたページが 10 ページ未満なら「完了」にしない
...
- 前面化は結果を確かめており、前面にできなかったときは `activate_failed` イベントに**何が前面にいたか**を残します（#74）。
  `--json` のログでこれが出ていれば、キー入力がそのウィンドウに吸われています
- 「予期しないエラー: …」で終わったときは、`--json` なら `error` イベントの `traceback`、それ以外なら標準エラー出力に
  トレースバックが出ます（#142）。issue に貼るときはこれを付けてください
