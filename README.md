...
- **縦書きの不可視テキスト**: 縦書きの列は 1 列まるごと 1 回で描きます（横書きは 1 文字ずつ）。
  1 文字ずつ独立に置くと PDF に「どれが同じ列か」の情報が残らず、読み順を幾何から推測する
  抽出器（`pdftotext` など poppler 系）が列をまたいで横に読みます。実測では OCR の行が
  そのまま取れる割合が poppler で 76/133 → 131/133 になりました（PDFium は描画順で読むため
  元から 133/133 で、こちらは 131/133 になります）。
  縦組み CMap の CID フォントは使いません。列はまとまりますが句読点と括弧が縦書き用の
  異体字になり、コピーすると `。「` が `︒ ﹁` になってしまうためです
