...
どちらかで、ページ番号を持つ本は開いた瞬間から「ページ」、持たない本はずっと「位置」です
（7 冊を headless で実測。撮影中に切り替わった本はありません）。切り替わるのは、位置 1 で開いた
あとにページ番号のデータが遅れて届く合本（ハリー・ポッター全 7 巻: `位置1/113618` → 5 回送って
`1/1358ページ`）だけで、このとき番号は振り直されます（455 → 5）。単位の違う 2 つの値を比べると
