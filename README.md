...
| 追加の依存 | なし | wordfreq（約 58MB。`pip install -e ".[verify]"`） |
...
- **行で切れた語**。テキスト層は行単位なので `テーブル` は `テーブ` + `ル` に切れます。
  断片は頻出語の先頭・末尾に一致するので見分けられます（`テーブ → テープ` を防ぐ）
- **候補が 2 つ以上ある語**。`パッグ` からは `バッグ`(4.44) と `パック`(4.48) の両方が
  閾値を超えます。頻度が最大のものを採ると `パック` ですが、実データの正解は `バッグ`
  でした（「キタムラのパッグ」）。決められないものは人に回します
- **1 度しか出ない語**
...
`--evidence frequency` は**語の境目に当たりうるルールを「要確認」として挙げます**。
`literal` は語の境界を見ない `str.replace` なので、短いキーは別の語の途中に当たります。
`カツプ → カップ` は `トンカツ` + `プレート` を `トンカップレート` にするため入れていません。
