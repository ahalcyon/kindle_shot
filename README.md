...
**画像が変わらなくなっても、読み手側の位置がまだ終わりでなければ、押さずに待ちます**
（#79）。10 秒 x 最大 3 回。実測で、`end_of_book` は本の途中でも出ます。合本 4 冊の
撮り直しでは位置 1237/2999（41%）と 14541/58503（25%）で「最終ページ」と判定されました。
**ところがその本は壊れていません。** あとから同じ位置を開いて送ると普通に進み
（1239, 1241, 1243...）、止まった地点の前後 16 ページはすべて別画像でした。

**押さずに待つ**のが要点です。再送ループはキーを押すので、描画が遅れているだけ
だった場合は押した分だけ本が進み、**ページが飛びます**。飛んだページは後段で
拾えません（`validate` が見るのは白紙・重複・寸法だけで、欠けは見ません）。
短い本より、中抜けの本のほうが悪いからです。

**待てば直るかどうかは未確認です。** 実機で観測したのは「本は壊れていない」ことと
「押したあと位置と画像は同じ 120ms のサンプルで変わる（描画の遅れではない）」ことまでで、
**待ったら直った、は見ていません**。停滞の正体は分かっていません。押さずに待つ形なので、
効かなくても従来と同じ結果になるだけで悪化はしません。効いたかどうかは次のバッチの
`capture_stalled` と `capture_stopped` で分かります。

位置が総量に達していれば待ちません。本当の最終ページで毎回 30 秒待つと 342 冊で
3 時間増えます。実測では最後まで撮れた本が `position == book_total` ちょうどで
終わっていました（**ただし 4 冊中 1 冊しか標本がありません**）。届かないまま正常に
終わる本があれば、その本は毎回 30 秒余計に待ったうえで結局 `end_of_book` になります
（失敗はしませんが遅くなります）。**次のバッチで `capture_stalled` の件数と
`position`/`book_total` を必ず集計すること。**

粘った箇所は `manifest.json` の `stalls` に残ります。中抜けの疑いがある本の目印です。
