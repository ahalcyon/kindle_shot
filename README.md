...
位置が総量の 98% に達していなければ最終ページとは呼びません（`short_of_end`、終了コード 1）。

実測 9 冊で決めた閾値です。最後まで撮れた本は位置が総量にほぼ到達します。
...
48% と 99.9% の間が大きく空いているので 0.98 に置きました。**到達しない本が実在すれば
その本は毎回失敗します**が、黙って途中までの本を完成扱いにするよりはよい、という判断です
（`batch` は出力があるとスキップするので、完成扱いは確定します）。次のバッチで
`capture_stopped` の `position` / `book_total` を必ず集計してください。
