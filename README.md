...
巻き戻しは**まず目次の先頭項目へ飛び**、残りをキーで詰めます（#70）。合本
（「シリーズ全 7 巻」など）はページ送りキーで巻の境界を越えられず、キーだけでは
本の先頭に戻せません（実測: ハリー・ポッター全 7 巻は位置 3216 付近から先へ
戻れず、397 回押しても 3634 で止まった）。目次からは 1 回で位置 1 まで飛べます。
普通の本でも数百回のキー送りが 1 回で済みます。

目次から飛んだ直後は**キー入力が効かなくなります**（実測: 8 回押しても位置が
動かず、「次のページ」ボタンだけは効いた）。パネルを Escape で閉じ、描画領域を
実際にクリックすると戻ります。目次を持たない本ではこれまでどおりキーだけで戻します。
