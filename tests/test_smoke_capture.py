...
def _fake_run_with_cover(tmp_path, monkeypatch, reason):
    ...
    with open(
        ...
        json.dump({"total_pages": 3, "stopped_reason": "max_pages", "duration_seconds": 1.0}, f)
    ...
    monkeypatch.setattr(
        smoke_capture.subprocess,
        "run",
        lambda *a, **kw: types.SimpleNamespace(returncode=0, stdout=stdout + "\n", stderr=""),
    )
    problems, _, _ = smoke_capture.run_smoke("B0TEST", out, 3, echo=lambda *_: None)
    return problems


def test_a_cover_already_on_page_one_is_not_a_failure(tmp_path, monkeypatch):
    """画面経路は先頭ページ＝表紙から撮るので、表紙を足さない（already_cover）のが正しい。

    取れなかった（外部サービスの都合）はこれまでどおり赤にする（#27 の偽陰性を戻さない）。
    """
