...
def test_map_keeps_the_default_shift_when_no_title_is_found_anywhere():
    """どのずれ幅でも章名が見つからない本は、全項目が既定のずれ幅（表紙の分） (#122)。

    「証拠の無い区間は作らない」を入れても、証拠がどこにも無い本はこれまでどおり。
    """
    ranges = [[p, p + 9] for p in range(0, 50, 10)]
    entries = map_to_pages(
        _chapters(5), ranges, 6, page_text=lambda i: "本文だけのページ", offset=1
    )
    assert [e.shift for e in entries] == [1] * 5
    assert [e.page for e in entries] == [1, 2, 3, 4, 5]
    assert all(e.how == UNCONFIRMED for e in entries)


def test_map_puts_trailing_entries_without_evidence_in_the_last_section():
    """末尾の証拠の無い項目は、直前の区間に付く（既定のずれ幅に戻らない） (#122)。"""
    ranges = [[p, p + 9] for p in range(0, 80, 10)]
    texts = [""] * 9
    for i in range(1, 6):  # 前半 5 章は 1 ページ後ろで見つかる
        texts[i] = f"なまえ{i:02d}"
    entries = map_to_pages(_chapters(8), ranges, 9, page_text=lambda i: texts[i])
    assert [e.shift for e in entries] == [1] * 8  # 末尾 3 章も 1 のまま
    assert [e.page for e in entries] == [1, 2, 3, 4, 5, 6, 7, 8]
