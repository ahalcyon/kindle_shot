...
def test_trim_auto_dry_run(pages_folder, capsys):
    code, events = run_cli(
        capsys,
        ["trim", "--in", str(pages_folder), "--auto", "--dry-run", "--json"],
    )
...
def test_trim_auto_reports_ui_bands_progress_with_minimum_pages(tmp_path, capsys):
    ...
    code, events = run_cli(
        capsys,
        ["trim", "--in", str(folder), "--auto", "--dry-run", "--json"],
    )
...
def test_trim_manual_margins_clipped_is_blocked(pages_folder, capsys):
    code, events = run_cli(
        capsys,
        ["trim", "--in", str(pages_folder), "--margins", "60,0,0,0", "--dry-run", "--json"],
    )
...
def test_trim_manual_margins_passthrough_copies_full_bleed_only(cover_folder, tmp_path, capsys):
...
def test_trim_auto_min_margins_does_not_passthrough_body(cover_folder, tmp_path, capsys):
...
def test_trim_empty_input_folder(tmp_path, capsys):
    ...
    code, events = run_cli(
        capsys,
        ["trim", "--in", str(empty), "--auto", "--dry-run", "--json"],
    )
...
def test_validate_strict_turns_warnings_into_errors(tmp_path, capsys):
    ...
    code, events = run_cli(
        capsys,
        ["validate", "--in", str(folder), "--strict", "--json"],
    )
