...
def cmd_check(args, rep):
    ...
    for name, ok, msg in checks:
        ...
        rep.event("check", human=f"[{mark}] {name}: {msg}", name=name, ok=ok, message=msg)
    rep.event(
        "result",
        human=("環境チェック: すべてOK" if all_ok else "環境チェック: 問題あり"),
        ok=all_ok,
    )
...
def build_parser():
    ...
    src.add_argument(
        "--asin", metavar="ASIN", help="本の ASIN（read.amazon.co.jp/?asin=<ASIN> を開く）"
    )
    ...
    p_open.add_argument(
        "--page-turn",
        choices=list(PAGE_TURN_KEYS),
        help="次ページのキー（right/left/pagedown/pageup/down/up。"
        "ブラウザ型ビューアの上下めくりは pagedown。"
        "巻き戻しはその逆を押す。既定はプロファイルの値）",
    )
    p_open.add_argument("--no-fullscreen", action="store_true", help="F11 全画面化をスキップ")
    p_open.add_argument(
        "--no-rewind",
        action="store_true",
    ...
    p_run.add_argument(
        "--title", required=True, help="タイトル（保存フォルダ名・出力ファイル名になる）"
    )
    p_run.add_argument(
        "--out", dest="output", required=True, metavar="FOLDER", help="保存先フォルダ"
    )
    p_run.add_argument(
        "--asin", metavar="ASIN", help="指定すると Cloud Reader で本を開くところから実行"
    )
    ...
    p_run.add_argument(
        ...
        type=float,
        metavar="SEC",
        help="ページ変化検出のポーリング間隔を上書き（画像の描画が遅い場合は長めにする）",
    )
    p_run.add_argument(
        "--expect-pages", type=int, metavar="N", help="validate で確認する期待ページ数"
    )
    p_run.add_argument(
        "--max-pages", type=int, metavar="N", help="キャプチャの上限ページ数（暴走防止）"
    )
    p_run.add_argument(
        "--max-rewind",
        type=int,
    ...
    p_run.add_argument(
        "--overwrite", action="store_true", help="保存先・トリミング先の既存画像を削除してから実行"
    )
    p_run.add_argument(
        "--ocr-workers",
        type=int,
    ...
    p_batch.add_argument(
        "--min-margins", metavar="L,R,T,B", help="トリミングで最低限削る余白（全本の既定）"
    )
    p_batch.add_argument(
        "--no-ui-bands",
        dest="ui_bands",
    ...
    p_batch.add_argument(
        "--ocr-workers", type=int, metavar="N", help="ndlocr-lite の並列プロセス数（全本の既定）"
    )
    p_batch.add_argument(
        "--faithful",
        action="store_true",
    ...
    p_cap.add_argument("--title", required=True, help="タイトル（保存先のサブフォルダ名になる）")
    p_cap.add_argument(
        "--out",
        dest="output",
        required=True,
    ...
    p_cap.add_argument(
        "--page-wait", type=float, metavar="SEC", help="ページ変化検出のポーリング間隔を上書き"
    )
    p_cap.add_argument(
        "--max-pages",
        type=int,
    ...
    p_pdf.add_argument("--in", dest="input", required=True, metavar="PDF", help="入力PDFファイル")
    p_pdf.add_argument(
        "--out",
        dest="output",
        metavar="FOLDER",
    ...
    p_pdf.add_argument(
        "--format", default="png", choices=["png", "jpg"], help="画像形式（既定: png）"
    )
    p_pdf.add_argument(
    ...
    p_trim.add_argument(
        "--in", dest="input", required=True, metavar="FOLDER", help="入力画像フォルダ"
    )
    p_trim.add_argument(
        "--out", dest="output", metavar="FOLDER", help="出力フォルダ（--dry-run 時は省略可）"
    )
    group = p_trim.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--margins", metavar="L,R,T,B", help="余白ピクセルを直接指定（例: 100,100,80,120）"
    )
    group.add_argument("--auto", action="store_true", help="全ページ走査で余白を自動検出する")
    p_trim.add_argument(
        "--safety",
        type=int,
        default=8,
    ...
    p_trim.add_argument(
        ...
        default=12,
        metavar="PX",
        help="背景との輝度差しきい値（既定: 12）。余白検出と全面表示ページの判定に使う",
    )
    p_trim.add_argument(
        "--passthrough",
        dest="passthrough",
    ...
    p_trim.add_argument("--force", action="store_true", help="内容が切れるページがあっても実行する")
    p_trim.add_argument(
        "--overwrite", action="store_true", help="出力フォルダの既存画像を削除してから実行する"
    )
    p_trim.add_argument(
    ...
    p_conv.add_argument(
        "--out", dest="output", required=True, metavar="FOLDER", help="出力フォルダ"
    )
    p_conv.add_argument("--format", required=True, choices=FORMATS, help="出力形式")
    p_conv.add_argument(
        "--name", metavar="FILENAME", help="出力ファイル名（省略時は入力フォルダ名）"
    )
    p_conv.add_argument(
        "--no-preprocess",
        action="store_true",
    ...
    p_conv.add_argument(
        "--upscale", type=float, metavar="X", help="OCR 前処理の拡大倍率を上書き（例: 1.5）"
    )
    p_conv.add_argument(
        "--no-replacements", action="store_true", help="OCR 後の置換辞書適用を無効化"
    )
    p_conv.add_argument("--replacements-path", metavar="PATH", help="置換辞書 JSON のパスを上書き")
    p_conv.add_argument(
        "--no-bookmarks", action="store_true", help="章しおりの自動検出・埋め込みを無効化"
    )
    p_conv.add_argument(
        "--no-reflow",
        action="store_true",
    ...
    p_conv.add_argument(
        "--source", metavar="TEXT", help="Markdown フロントマターに記録する出典情報（ASIN 等）"
    )
    p_conv.add_argument(
        "--embed-images",
        action="store_true",
    ...
    p_val.add_argument(
        "--in", dest="input", required=True, metavar="FOLDER", help="検証する画像フォルダ"
    )
    p_val.add_argument(
        "--expect-pages",
        type=int,
    ...
    p_val.add_argument(
        "--strict", action="store_true", help="警告 (白紙・重複・サイズ違い) もエラー扱いにする"
    )
    ...
    p_chk.add_argument(
        "--profile", metavar="KEY", help="指定するとキャプチャ対象ウィンドウの検出まで確認する"
    )
