...
# フッターの単位 (#110)。ページ番号を持たない本はずっと「位置」。持つ本は、ページ番号の無い
# 表紙・前付けでは「位置」、本文に入ると「ページ」になり、番号も振り直される（実測: 作文技術は
# 位置1/3257 → 位置2/3257 → 1/265ページ、半導体の地政学は 位置1/3494 → 1/292ページ）。先頭から撮る
# 本番では**必ず**この切り替わりを通る。合本では本文の途中でも起きうる（ハリー・ポッター全 7 巻:
# 位置1/113618 → 5 回送って 1/1358ページ）。単位が違う 2 つの読み値を比べると「飛んだ」「戻った」と
# 読み違えるので、読み値には単位を持たせ、比べる側で揃っているかを見る。
UNIT_LOCATION = "位置"
UNIT_PAGE = "ページ"
...
def unsupported_reason(page, *, markers=UNSUPPORTED_MARKERS):
    ...
    しおりの作り直し（``core.bookmark_rebuild.cached_structure``。撮影の流れと
    ``scripts/rebuild_bookmarks.py`` の両方から来る）からも呼ぶ。あちらは判定を記録しないので、
    誤判定でも次回の実行でやり直せる（害の大きさが呼び出し元で違う）。
